...
class TestBuyExecutorPricing(unittest.TestCase):
    ...
    @patch.dict(os.environ, {
        "BUY_REQUIRE_LIVE_MARKET_MATCH": "0",
        "BUY_SKIP_BOOK_ENABLED": "1",
        "BUY_BURST_DEDUPE_ENABLED": "1",
        "BUY_BURST_DEDUPE_TTL_SECONDS": "30",
    }, clear=False)
    def test_burst_dedupe_skips_same_wallet_token_side_event_second(self) -> None:
        from v2_hybrid_signals.buy_config import parse_args
        from v2_hybrid_signals.buy_executor import BuyExecutor
        ...
        now = time.time()
        event_ts = float(int(now))
        first = executor.process_actionable(
            {
                "match_key": "burst-1",
                "source": "orderbook_subgraph",
                "tracked_user": "0xabc",
                ...
                "signal": {
                    ...
                    "price": 0.61,
                    "usdc_size": 10.0,
                    "condition_id": "cond-burst",
                },
            },
            live_markets=[],
        )
        second = executor.process_actionable(
            {
                "match_key": "burst-2",
                "source": "orderbook_subgraph",
                "tracked_user": "0xabc",
        ...
        self.assertEqual(first.get("status"), "dry_run_ok")
        self.assertEqual(second.get("decision"), "skip")
        self.assertEqual(second.get("reason"), "duplicate_burst_key")
    ...
    def test_burst_dedupe_allows_next_second(self) -> None:
        from v2_hybrid_signals.buy_config import parse_args
        from v2_hybrid_signals.buy_executor import BuyExecutor
        ...
        self.assertEqual(first.get("status"), "dry_run_ok")
        self.assertEqual(second.get("status"), "dry_run_ok")
